...
randomMotionBlurOpr = AugOperator(
    "Random Motion Blur", K.augmentation.RandomMotionBlur, 1.0,
    AugParam(Param("kernel_size", 5, DecDiscreteRange(3, 9)),
             Param("angle", 90, ContRange(15, 340)),
             Param("direction", 3, ContRange(0.1, 3.9))))

randomPosterizeOpr = AugOperator(
    "Random Poseterize", K.augmentation.RandomPosterize, 1.0,
    AugParam(Param("bits", 5, DecDiscreteRange(8, 5))))

colorJiggleOpr = AugOperator(
    "Color Jiggle", K.augmentation.ColorJiggle, 1.0,
    AugParam(Param("brightness", 1.4, ContRange(0.1, 2.0)),
             Param("contrast", 1.8, DecContRange(3.0, 0.2)),
             Param("saturation", 2.6, LoopContRange(0.1, 3.0))))

randomSharpnessOpr = AugOperator(
    "Random Sharpness", K.augmentation.RandomSharpness, 1.0,
    AugParam(Param("sharpness", 0.5, ContRange(0.01, 0.87))))

randomErasingOpr = AugOperator(
    "Random Erasing", K.augmentation.RandomErasing, 1.0,
    AugParam(Param("scale", (0.25, 0.25), TupleContRange(0.1, 0.3))))

randomResizedCropOpr = AugOperator(
    "Random Resied Crop", K.augmentation.RandomResizedCrop, 1.0,
    AugParam(Param("scale", (0.75, 0.75), TupleDecContRange(0.99, 0.75)),
