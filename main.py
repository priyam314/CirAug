...
from fusion.utils.util import loader, ifExistLoad, Print
...
trainModel = ifExistLoad(
    Encoder(cfg.model.encoder)(TrainProjector(cfg.hlayer)),
    pt, cfg.model.name, "train"
)
valModel = ifExistLoad(
    Encoder(cfg.model.encoder)(TrainProjector(cfg.hlayer)),
    pt, fuse.utils.util.trainOf(cfg.model.name), "fine-tune"
)
testModel = ifExistLoad(
    Encoder(cfg.model.encoder)(TestProjector(cfg.hlayer)),
    pt, fuse.utils.util.fineTuneOf(cfg.model.name), "test"
)
...
widgets = {
    'Augs':Augs, 'protrack':pt, 'optim': Optim(cfg.optimizer.name), 
    'criterion': Criterion(cfg), 'scheduler': Scheduler('yo')
}
pipe = PipeLine(cfg.exec.typ)
pipe.add(Train(model = trainModel, dataloader = train_dataloader, data = cfg.data, **widgets))
pipe.add(FineTune(model = valModel, projector = TestProjector(cfg.hlayer), dataloader = val_dataloader, data = cfg.data, **widgets))
pipe.add(Test(model = testModel, dataloader = test_dataloader, data = cfg.data, **widgets))
pipe.set_on_start()
