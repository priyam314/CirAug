...
class Executioner(ABC):
    ...
    def oneEpochRun(self, epoch):
        logger.info(f"Inside oneEpochRun for epoch: {epoch}")
        ...
        print ("*********")
        logger.info("Time Taken till Epoch {} is {:.2f}s".format(epoch, time_epoch_start.elapsed_time(time_epoch_end)/1000))
        print ("*********")
